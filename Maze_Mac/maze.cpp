...
void setOrthographicProjection() {
	...
	for (int i = 0; i < numSnowMen; ++i)
	{
		for (int j = 0; j < NUM_BULLETS; ++j)
		{
			...
			if(abs((int)(actualBulletX[j] - Snowmen[i].x)) <= 50 && abs((int)(actualBulletZ[j]-Snowmen[i].z)) <= 50){
		...
		if(Snowmen[i].aLive){
		if(abs((int)(Snowmen[i].actualCarrotPosX-camx))<= 50 && abs((int)(Snowmen[i].lastZ - camz)) <= 50){
			life -= 5;
			...
			if(abs((int)(Snowmen[i].actualCarrotPosZ-camz))<= 50 && abs((int)(Snowmen[i].lastX - camx)) <= 50){
			life -= 5;
			Snowmen[i].fired = false;
			Snowmen[i].carrotPos = 0;
			Snowmen[i].Draw();
...
void MyKeyboard(int thekey, int mouseX, int mouseY)
{
	switch(thekey)
	{
		...
		case GLUT_KEY_ENTER: {OpenDoor();break;}
		case GLUT_KEY_HOME: {SwitchCAM();break;}
		case 's': {gameState = PLAY;break;}
		case GLUT_KEY_SPACE: {if(bulletsLeft>0){ cout << bulletsLeft << endl; bulletsLeft = bulletsLeft-1;gunFired[bulletsLeft]= true;
